...
def generate_aws_exports(
    ...
    aws_exports = {
        ...
        # sre-four-agent uses LangGraph internally, so the frontend parser
        # needs "langgraph-single-agent" to correctly interpret streaming responses
        "agentPattern": "langgraph-single-agent" if pattern == "sre-four-agent" else pattern,
    }

    # Merge any extra config (e.g. logGeneratorApiUrl)
    if extra_config:
        aws_exports.update(extra_config)
...
def main() -> int:
    ...
    # Parse extra config from command line (--extra-config key=value key2=value2)
    extra_config = {}
    if "--extra-config" in sys.argv:
        idx = sys.argv.index("--extra-config")
    ...
    try:
        generate_aws_exports(stack_name, outputs, region, pattern, frontend_dir, extra_config or None)
    except ValueError as e:
        log_error(str(e))
        return 1
