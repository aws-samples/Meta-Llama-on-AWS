...
def build_graph(mcp_tools: list, max_revisions: int = 3, shortcut_quality_score: float = 4.95, graph_filename: str | None = None, max_tokens: int = 8192, use_builtin_tools: bool = True):
    ...
    llm = ChatSagemakerWithTools(
        ...
        model_kwargs={"temperature": 0.1, "max_tokens": max_tokens}  # Increased for agent workflows with tool calling
    )
...
async def run_cli(evaluate_mode: bool = False, max_revisions: int = 0, shortcut_quality_score: float = 4.95, graph_filename: str | None = None, max_tokens: int = 8192):  
    ...
    graph = build_graph(mcp_tools=mcp_tools, max_revisions=max_revisions, shortcut_quality_score=shortcut_quality_score, graph_filename=graph_filename, max_tokens=max_tokens)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-tokens", type=str, default=8192,
                        help="Maximum number of context tokens (optional)")
