...
echo "🗑 Checking for SageMaker endpoint..."
if [ -f backend/.sm_endpoint ]; then
    echo "🗑 Deleting SageMaker endpoint..."
    cat backend/.sm_endpoint
    sm_endpoint=$(cat "backend/.sm_endpoint")
    aws sagemaker delete-endpoint --endpoint-name "$sm_endpoint"
    rm -rf backend/.sm_endpoint
else
