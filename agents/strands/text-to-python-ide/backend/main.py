...
code_gen_system_prompt = f"""You are a Python code generator specialist powered by Llama 3.3 70B model. Your role is to:
...
"""
CODE_EXE_SYSTEM_PROMPT = f"""You are a helpful AI assistant powered by Llama 3.3 70B model that validates all answers through code execution.
...
def smjs_endpoint():
    print("Creating SageMaker endpoint for llama-3-3-70b-instruct")
    # Fetch the IAM role created by SageMaker
    sm_role = [r for r in iam.list_roles()["Roles"] if r["RoleName"].startswith("AmazonSageMaker-ExecutionRole")][0]["Arn"]
    model_id, model_version = "meta-textgeneration-llama-3-3-70b-instruct", "*"
    accept_eula = True
    model = JumpStartModel(model_id=model_id, model_version=model_version, region="us-west-2", role=sm_role)
    predictor = model.deploy(accept_eula=accept_eula, instance_type="ml.p4d.24xlarge")
    with open('.sm_endpoint', 'w') as ep:
        ep.write(predictor.endpoint_name + '\n') 
    ep.close()
...
@lru_cache(maxsize=3)
def create_model_with_fallback(aws_region: str):
    ...
    # Primary model: 
    if primary == "sm":
        sm_endpoint = smjs_endpoint()
        primary_code_gen_model_id = "SM-meta-textgeneration-llama-3-3-70b-instruct" 
        primary_code_exe_model_id = "us.meta.llama4-maverick-17b-instruct-v1:0"
        print(f"🤖 Attempting to use primary inference profiles: {primary_code_gen_model_id} on SM and {primary_code_exe_model_id} on BR")
    else:	
    	primary_model_id = "us.meta.llama3-3-70b-instruct-v1:0"
    fallback_model_id = "us.meta.llama4-maverick-17b-instruct-v1:0"
    default_model_id = "us.meta.llama3-3-70b-instruct-v1:0"
    
    
    print(f"primary model id: {primary}")
    # Try primary model (inference profile)
    try:
        if primary == 'sm' :
            primary_code_gen_model = SageMakerAIModel(
                            endpoint_config={
            ...
            #primary_model_id = "us.meta.llama3-3-70b-instruct-v1:0" 
            print(f"✅ Primary code gen inference profile {primary_code_gen_model_id} on SageMaker initialized successfully")

            primary_code_exe_model = BedrockModel(
                model_id=primary_code_exe_model_id,
                aws_region=aws_region,
                streaming=False,
                botocore_config=get_extended_botocore_config()
            )

            print(f"✅ Primary code exe inference profile {primary_code_exe_model_id} on Bedrock initialized successfully")
            result = (primary_code_gen_model, primary_code_exe_model, primary_code_gen_model_id, primary_code_exe_model_id)
            return result
        else:
            ...
            result = (primary_model, primary_model, primary_model_id, primary_model_id)
...
def initialize_agents():
    ...
    global code_generator_agent, code_executor_agent, executor_type, current_gen_model_id, current_exe_model_id
    ...
    if 'code_generator_agent' in _agents_cache and 'code_executor_agent' in _agents_cache:
        ...
        current_gen_model_id = _agents_cache['current_gen_model_id']
        current_exe_model_id = _agents_cache['current_exe_model_id']
    ...
    try:
        ...
        #model, model_id = create_model_with_fallback(aws_region)
        code_gen_model, code_exe_model, code_gen_model_id, code_exe_model_id = create_model_with_fallback(aws_region)
        print(f"🎯 Using code generation model: {code_gen_model_id}")
        print(f"🎯 Using code execution model: {code_exe_model_id}")
        ...
        if primary == 'sm':
            ...
            code_executor_agent = Agent(
                model=code_exe_model,
                tools=[execute_python_code],
                system_prompt=CODE_EXE_SYSTEM_PROMPT
            )
        
        print("✅ Agents initialized successfully:")
        print(f"   - Code Generator: Strands-Agents Agent with {code_gen_model_id}")
        print(f"   - Code Executor: Strands-Agents Agent with {code_exe_model_id} + AgentCore CodeInterpreter")
        
        # Cache the agents
        current_gen_model_id = code_gen_model
        current_exe_model_id = code_exe_model
        _agents_cache['code_generator_agent'] = code_generator_agent
        _agents_cache['code_executor_agent'] = code_executor_agent
        _agents_cache['current_gen_model_id'] = current_gen_model_id
        _agents_cache['current_exe_model_id'] = current_exe_model_id
...
"""
        ...
        if primary == "sm":
            sys_prompt = code_gen_system_prompt
        else:
            sys_prompt = ""
        print(f"generate_code: {sys_prompt}")
        agent_result = code_generator_agent(f"{sys_prompt} {enhanced_prompt}")
        print(agent_result)
...
Keep response short and practical."""
            ...
            if primary == "sm":
                ...
                sys_prompt = ""
            print(f"Analyze code: {sys_prompt}")
            analysis_result = code_generator_agent(f"{sys_prompt} {analysis_prompt}")
...
Use the tool to run the code and return the complete output."""               
                if [ primary =='sm' ]:
                    sys_prompt = CODE_EXE_SYSTEM_PROMPT
                else:
                    sys_prompt = ""
                
                execution_result = code_executor_agent(f"{sys_prompt} {execution_prompt}")
...
async def get_agents_status():
    ...
    try:
        current_gen_model = globals().get('current_gen_model_id', 'Unknown')
        current_exe_model = globals().get('current_exe_model_id', 'Unknown')
        agents_info = [
            {
                "name": "code_generator",
        ...
        architecture = f"Hybrid: Strands-Agents + AgentCore ({current_gen_model})" if executor_type == "agentcore" else f"Strands-Agents Framework ({current_exe_model})"
        ...
        return {
            ...
            "current_gen_model": current_gen_model,
            "current_exe_model": current_exe_model,
            "aws_region": aws_region,
            ...
            if message["type"] == "generate_code":
                ...
                # Handle code generation via WebSocket
                    if primary == "sm":
                        ...
                        sys_prompt = ""
                    print(f"Websocket: {sys_prompt}")
                    agent_result = code_generator_agent(f"{sys_prompt} {message["prompt"]}")
                    # Extract string content from AgentResult
                    generated_code = str(agent_result) if agent_result is not None else ""
                ...
                try:
                    ...
                    if executor_type == "agentcore":
                        execution_result = code_executor_agent(f"Execute this code: {sys_prompt} {message['code']}")
                    else:
                        execution_result = code_executor_agent(f"Simulate execution of: {sys_prompt} {message['code']}")
...
async def health_check():
    ...
    return {
        ...
        "authentication": "AWS Profile" if os.getenv('AWS_PROFILE') else "Access Keys",
        "architecture": {
            "code_generation": f"Strands-Agents Agent ({current_gen_model})",
            "code_execution": f"{executor_type.title().replace('_', ' ')} Agent ({current_exe_model})"
        }
    }
